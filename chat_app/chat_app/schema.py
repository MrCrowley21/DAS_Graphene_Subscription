...
from chat_app.serializer import ChatType, ChatFilter, MessageType, MessageFilter
...
class Query(MeQuery, graphene.ObjectType):
    ...
    chat_field = graphene.Field(ChatType, id=graphene.ID())
    ...
    @staticmethod
    def resolve_chat(cls, info, c_id, **kwargs):
        user = info.context.user
        return Chat.objects.prefetch_related("participants").get(participants=user, id=c_id)

    @staticmethod
    def resolve_messages(cls, info, m_id, **kwargs):
        user = info.context.user
        chat = Chat.objects.prefetch_related("messages", "participants").get(participants=user, id=m_id)
        return chat.messages.all()


class CreateChat(graphene.Mutation):
    # pass here emails (valid users)
...
class OnNewMessage(channels_graphql_ws.Subscription):
    ...
    def publish(self, info, chatroom=None):
        return OnNewMessage(message=self)
...
class SendMessage(graphene.Mutation):
    ...
    def mutate(cls, _, info, message, chat_id):
        user = info.context.id
        chat = Chat.objects.prefetch_related("participants").get(participants=user, id=chat_id)
        message = Message.objects.create(sender=user, text=message, created=now())
...
schema = graphene.Schema(query=Query, mutation=Mutations, subscription=Subscriptions)
